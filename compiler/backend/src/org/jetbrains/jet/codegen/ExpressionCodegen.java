...
public class ExpressionCodegen extends JetVisitor<StackValue, StackValue> implements LocalLookup {
    ...
    private StackValue generateTopLevelPropertyReference(@NotNull VariableDescriptor descriptor) {
        ...
        if (receiverParameter != null) {
            factory = descriptor.isVar()
                      ? method("mutableExtensionProperty", K_MUTABLE_EXTENSION_PROPERTY_IMPL_TYPE, JAVA_STRING_TYPE, K_PACKAGE_IMPL_TYPE,
                               getType(Class.class))
                      : method("extensionProperty", K_EXTENSION_PROPERTY_IMPL_TYPE, JAVA_STRING_TYPE, K_PACKAGE_IMPL_TYPE,
                               getType(Class.class));
        }
        else {
            factory = descriptor.isVar()
                      ? method("mutableTopLevelProperty", K_MUTABLE_TOP_LEVEL_PROPERTY_IMPL_TYPE, JAVA_STRING_TYPE, K_PACKAGE_IMPL_TYPE)
                      : method("topLevelProperty", K_TOP_LEVEL_PROPERTY_IMPL_TYPE, JAVA_STRING_TYPE, K_PACKAGE_IMPL_TYPE);
        }
    ...
    private StackValue generateMemberPropertyReference(@NotNull VariableDescriptor descriptor) {
        ...
        Method factory = descriptor.isVar()
        ...
        if (containingClass instanceof JavaClassDescriptor) {
            ...
            v.invokestatic(REFLECTION_INTERNAL_PACKAGE, "foreignKotlinClass",
                           Type.getMethodDescriptor(K_FOREIGN_CLASS_TYPE, getType(Class.class)), false);
        }
        else {
            v.getstatic(classAsmType.getInternalName(), JvmAbi.KOTLIN_CLASS_FIELD_NAME, K_KOTLIN_CLASS_TYPE.getDescriptor());
        }

        v.visitLdcInsn(descriptor.getName().asString());
        v.invokeinterface(K_CLASS_IMPL_TYPE.getInternalName(), factory.getName(), factory.getDescriptor());
